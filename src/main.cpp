...
uint256 hashGenesisBlock("0x8cb5b97deab4f04ada1b242c8b95a14de9eb07874e39beacb23aa4d867491a97");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bunnycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bunnycoin Signed Message:\n";
...
{
    // Bunnycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    else if(nHeight > 600000)
    {
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand6 = generateMTRandom(seed, 9999);
        nSubsidy = (1 + rand6) * COIN;
    }

    return nSubsidy*0.9 + nFees;

}

int64 tempGrantValue;
int64 grantValue;
int interval = 60;
...
{
    tempGrantValue=0;
    CBlockIndex * gdBlockPointer;
    gdBlockPointer=pindexBest;

    CBlock block;
    for (int a=0; a < interval-1;a++){
        gdBlockPointer=gdBlockPointer->pprev;
    }

    block.ReadFromDisk(gdBlockPointer);
    tempGrantValue = block.vtx[0].vout[0].nValue;
    gdBlockPointer=gdBlockPointer->pprev;
    ...
    for (int a=0; a < interval-1;a++){
        ...
        for (unsigned int i = 0; i <block.vtx.size(); i++){
                if(block.vtx[i].IsCoinBase()){
                    tempGrantValue +=block.vtx[i].GetValueOut();
                }
        }
        gdBlockPointer=gdBlockPointer->pprev;
    }

    return (tempGrantValue*10)/90;
}


static const int64 nTargetTimespan = 4*60 * 60; // adjust difficulty based on the past 4 hours
static const int64 nTargetSpacing = 60; //  minute
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
...

    {
        ...
        if(isGrantAwardBlock(pindex->nHeight)){
            ...
            printf("Grant award in block %d, %zu\n",awardFound,grantAwards.size());
             for(gait=grantAwards.begin(); gait!=grantAwards.end(); ++gait){
        ...
        int64 blockValue =GetBlockValue(pindex->nHeight, nFees+grantAward,prevHash);
        int64 getValueOut = vtx[0].GetValueOut();
        //Check correct amount awarded in grants
        if(pindex->nHeight==1 && !fTestNet){
          //  if(vtx[0].GetValueOut()!=75567060000000){
          //      return state.DoS(100, error("ConnectBlock() : Block 1 coinbase pays too much (actual=%"PRI64d" vs limit=%"PRI64d")", getValueOut, blockValue));
          //1398291000  }
        }else if (getValueOut > blockValue){
            return state.DoS(100, error("ConnectBlock() %d : coinbase pays too much (actual=%"PRI64d" vs limit=%"PRI64d")", pindex->nHeight, getValueOut, blockValue));
        }
    }
...
{
    // Bunnycoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8cb5b97deab4f04ada1b242c8b95a14de9eb07874e39beacb23aa4d867491a97");
    }
...
bool InitBlockIndex() {
   ...
   // fMultiAddress = GetBoolArg("-multiaddress", false);
    fMultiAddress = GetBoolArg("-multiaddress", true);
   ...
   // fTxIndex = GetBoolArg("-txindex", false);
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "25/04/2014 I had to enter something here";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1398441600;
        block.nBits    = bnProofOfWorkLimit.GetCompact();

       // block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2520830;
        ...
        assert(block.hashMerkleRoot == uint256("0x4c87c96563156ca048b0062439bdea368717e2b65425e513d32f02a571f12f70"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BunnycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BunnycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BunnycoinMiner(CWallet *pwallet)
{
    printf("BunnycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bunnycoin-miner");
    ...
    try { loop {
        ...
        printf("Running BunnycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BunnycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BunnycoinMiner, pwallet));
}
...
//static const int64 GRANTBLOCKINTERVAL = (0.75*60*60)/nTargetSpacing;// every 45 block
static const int64 GRANTBLOCKINTERVAL = (1*60*60)/nTargetSpacing;// every 60 block
// BUNNchaiaxxaS3DEcb894TomLT3VxxCcmK
static string GRANTPREFIX ="BUNN"; // BUNNcha
static int numberOfOffices = 1;
string electedOffices[2];
...
std::map<std::string,std::map<int64,std::string> > votingPreferences[2]; //Voting prefs as at grant allocation block point
...
bool ensureGrantDatabaseUptoDate(int64 nHeight){
    ...
    if(getGrantDatabaseBlockHeight()==-1){
        ...
        electedOffices[0]="cha";
        electedOffices[1]=newCV;
    }

    //requiredgrantdatabaseheight is 60 less than the current block
...
//        if(grantVoteAddress.substr(2,3)==electedOffices[i]){
            return i;
        }
    }
    return -1;
...
void processNextBlockIntoGrantDatabase(){
    ...
    //// szukamy tranzakcji glosujacych
    for (unsigned int i = 0; i <block.vtx.size(); i++){
        ...
        for (unsigned int j = 0; j <block.vtx[i].vout.size(); j++){
            ...
            if(startsWith(receiveAddress.c_str(),GRANTPREFIX.c_str()) && theAmount<11 && theAmount>0){
...
void printBalances(int64 howMany, bool printVoting, bool printWasted){
    ...
    printf("%zu addresses with balances. Printing Top %llu\n",balances.size(),howMany);
...
bool getGrantAwardsFromDatabaseForBlock(int64 nHeight){
    ...
    debugVote = GetBoolArg("-debugvote", true);
    if(debugVote){
...
for(int i=0;i<numberOfOffices+1;i++){
    ...
    if(i<numberOfOffices){
        int64 getGrantValue = GetGrantValue();
        for(int i=0;i<getNUMBEROFAWARDS(nHeight);i++){ // zwraca zawsze 1
            grantAwards[awardWinners[i]]=grantAwards[awardWinners[i]]+getGrantValue*howManyPercent(i);
            if(debugVote)grantAwardsOutput << "Add grant award to Block "<<awardWinners[i].c_str()<<" ("<<getGrantValue*howManyPercent(i)/COIN<<")\n";
        }
    }
...
void getWinnersFromBallots(int64 nHeight, int officeNumber){
    ...
    for(int i=getNUMBEROFAWARDS(nHeight);i>0;i--){
        ...
        if(debugVoteExtra)printBallots();
        do{
            if(debugVote)grantAwardsOutput <<"-------------:\nElimination Round:"<<voteRoundNumber<<"\n";
...
string	electOrEliminate(int64 droopQuota, unsigned int requiredCandidates){
    ...
    if(topOfThePollAmount==0){
        //No ballots left -end -
        if(debugVote)grantAwardsOutput <<"No Candidates with support remaining. Grant awarded to unspendable address BUNNchaiaxxaS3DEcb894TomLT3VxxCcmK\n";
        return "BUNNchaiaxxaS3DEcb894TomLT3VxxCcmK";
    }
